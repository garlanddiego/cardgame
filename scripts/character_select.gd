...
## 5 hero portrait cards in a row, single/dual mode toggle, slot-based selection
...
var _next_slot: int = 0  # 0 = fill slot1, 1 = fill slot2 (dual mode alternating)
...
func _build_ui() -> void:
	...
	# === TOP BAR: back button + mode toggle ===
	...
	back_btn.text = "← 返回"
	back_btn.custom_minimum_size = Vector2(100, 45)
	...
	# === BOTTOM BAR: selection slots + start button ===
	...
	_start_btn.custom_minimum_size = Vector2(160, 55)
...
func _create_hero_card(hero_id: String) -> PanelContainer:
	...
	style.bg_color = Color(color.r * 0.15, color.g * 0.15, color.b * 0.15, 0.9)
	style.border_color = Color(color.r * 0.5, color.g * 0.5, color.b * 0.5, 0.6)
	...
	btn_overlay.pressed.connect(func(): _on_hero_selected(hero_id))
...
func _create_slot(label_text: String) -> Array:
	...
	slot_style.border_color = Color(0.4, 0.35, 0.25)
	...
	slot_bg.custom_minimum_size = Vector2(55, 55)
	...
	name_lbl.add_theme_color_override("font_color", Color(0.85, 0.75, 0.6))
	...
	if _dual_mode:
		if _next_slot == 0:
			...
			if hero_id == _selected_hero1:
				return  # Can't pick same hero twice
