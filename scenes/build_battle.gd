...
func _initialize() -> void:
	...
	# Draw pile panel (bottom left) — clickable
	var draw_panel = Panel.new()
	draw_panel.name = "DrawPanel"
	draw_panel.position = Vector2(30, 880)
	draw_panel.size = Vector2(160, 50)
	var draw_style = StyleBoxFlat.new()
	draw_style.bg_color = Color(0.1, 0.15, 0.25, 0.85)
	draw_style.border_color = Color(0.3, 0.5, 0.8)
	draw_style.set_border_width_all(1)
	draw_style.set_corner_radius_all(6)
	draw_style.content_margin_left = 10
	draw_style.content_margin_top = 6
	...
	# Discard pile panel (bottom right) — clickable
	var discard_panel = Panel.new()
	discard_panel.name = "DiscardPanel"
	discard_panel.position = Vector2(1700, 880)
	discard_panel.size = Vector2(190, 50)
	var discard_style = StyleBoxFlat.new()
	discard_style.bg_color = Color(0.25, 0.1, 0.1, 0.85)
	discard_style.border_color = Color(0.8, 0.4, 0.4)
	discard_style.set_border_width_all(1)
	discard_style.set_corner_radius_all(6)
	discard_style.content_margin_left = 10
	discard_style.content_margin_top = 6
